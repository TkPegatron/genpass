...
pub fn hash_sha512(password: String) -> String {
    ...
    let hashed_password: String =
        sha512_simple(password.as_str(), &params).expect("Should not fail");
...
mod tests {
    ...
    fn test_argon2_hash() {
        ...
        assert!(Argon2::default()
            .verify_password(TEST_PASSWORD.as_bytes(), &password_hash)
            .is_ok())
    }
    ...
    fn test_sha512_hash() {
        assert!(sha512_check(
            TEST_PASSWORD,
            hash_sha512(TEST_PASSWORD.to_string()).as_str()
        )
        .is_ok());
    }
}
